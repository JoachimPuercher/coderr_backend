...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        # rule, this one turns it into a 400
        user = attrs["business_user"]
        reviewer = self.context["request"].user
        exists = Review.objects.filter(
            reviewer=reviewer, business_user=user).exists()
        if exists:
            raise serializers.ValidationError(
...
class ReviewUpdateSerializer(ReviewSerializer):
    ...
    class Meta(ReviewSerializer.Meta):
        read_only_fields = (
            ReviewSerializer.Meta.read_only_fields + ["business_user"]
        )

    def validate(self, attrs):
        """Answer unknown or read-only keys
        with a 400 instead of ignoring them.
        """
        # replaces the duplicate check of the parent,
        # which needs business_user in the payload
